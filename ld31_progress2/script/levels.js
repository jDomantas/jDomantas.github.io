...
var levels = {
					...
					if (tile.pressed)
						game.loadLevel(app, levels.level2, false, function(game, x, y) 
							{ 
								...
								if (y == game.height - 5) return (game.width - x) / 40 + 0.65;
								if (x == 0) return (game.height - y) / 40 + 1.15;
								return 1.3;
							});
				}
			},
	...
	level11: createLevel([
		...
		[ 
			{
				action: function(app, game) {
					game.message(app, 'Congrats. More levels incoming (*hopefully*)');
				}
			}
		])
};
